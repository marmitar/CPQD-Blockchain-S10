...
contract CPQD {
     ...
     * @param resultHash Keccak-256 hash of the pair `(value, salt)`, where `value` is the secretly chosen bet result
    ...
    /**
     * @notice Reveal bet results for the users. The hash of `(value, salt)` must match previously committed hash.
